...
test.describe("KICK - kick-counter", () => {
  ...
  test("KICK2-17: Medical disclaimer renders with clear escalation guidance", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
    await expect(page.getByText(/not a medical device|contact.*healthcare provider/i)).toBeVisible();
  });
  ...
  test("KICK2-19: Full authenticated app sidebar renders consistently", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
  ...
  test("KICK2-18: Page <title> is unique to Kick Tracker", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
    await expect(page).toHaveTitle(/Kick.*Numaa/i);
  });
  ...
  test.skip("KICK2-13: Chart renders a 7-day rolling window ending today", async () => {
    requireLiveFeature('deterministic chart data', 'Requires deterministic chart data and a stable date/time fixture');
  });
  ...
  test("KICK2-14: Day/Week/Month range toggle renders", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
    for (const label of ['Day', 'Week', 'Month']) await expect(page.getByRole('button', { name: label, exact: true })).toBeVisible();
  });
  ...
  test.skip("KICK2-15: Switching range updates the chart's date span appropriately", async () => {
    requireLiveFeature('stable chart data for range comparison', 'Requires stable chart data for comparing rendered date spans');
  });
  ...
  test.skip("KICK2-16: Chart reflects real kick data once logged", async () => {
    requireCredentials();
  });
  ...
  test("KICK2-01: Page loads with heading, description, and CTA", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
  ...
  test("KICK2-02: Generic \"Chat\" CTA has a descriptive accessible label", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
  ...
  test("KICK2-09: Chart renders with all kick-type categories and range toggle", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
    for (const label of ['Gentle', 'Flutter', 'Pop', 'Strong', 'Roll', 'Indescribable']) await expect(page.getByText(label, { exact: true })).toBeVisible();
  });
  ...
  test.skip("KICK2-10: Chart axis scale is sensible when there is no data", async () => {
    requireCredentials();
  });
  ...
  test.skip("KICK2-11: Day/Week/Month toggle changes the chart's time range", async () => {
    requireLiveFeature('deterministic chart range data', 'Requires deterministic chart data for range comparison');
  });
  ...
  test.skip("KICK2-12: Legend items toggle visibility of a kick-type series", async () => {
    requireCredentials();
  });
  ...
  test("KICK2-03: Today's date renders correctly", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
  ...
  test("KICK2-04: Kick counter and empty-state summary render", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
    await expect(page.getByText(/Total Kicks Today/i)).toBeVisible();
    await expect(page.getByText(/Your Kick Count Summary/i)).toBeVisible();
  });
  ...
  test("KICK2-05: \"View History\" and \"Start Session\" controls render", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
  ...
  test.skip("KICK2-06: \"Start Session\" begins a live kick-logging session \u2014 HIGH PRIORITY", async () => {
    requireLiveFeature('live kick-count session', 'Requires starting a live mutable kick-count session');
  });
  ...
  test.skip("KICK2-07: \"View History\" opens a full kick-count history view", async () => {
    requireCredentials();
  });
  ...
  test.skip("KICK2-08: Logging a kick updates the counter and summary in real time", async () => {
    requireLiveFeature('mutable kick-count session', 'Requires mutating kick-count data during a live session');
  });
  ...
  test("KICK-01: \"What is Kick Counting?\" info renders", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
    await expect(page.getByText('What is Kick Counting?', { exact: true })).toBeVisible();
    await expect(page.getByText(/Why Monitor Kick Counts\?/i)).toBeVisible();
  });
  ...
  test("KICK-02: \"Quick Guidelines\" 3-step list renders in order", async ({ page }) => {
    await login(page);
    await page.goto('/baby-kick-tracker');
    for (const label of ['Choose a Time', 'Count Movements', 'Record & Share']) await expect(page.getByText(label, { exact: true })).toBeVisible();
  });
  ...
  test.skip("KICK-03: \"Learn More\" link opens the full guide", async () => {
    requireLiveFeature('kick-count guide navigation', 'Requires navigation to a separate guide and validating its account-independent content');
  });
