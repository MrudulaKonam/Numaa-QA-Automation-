...
test.describe("Service - service", () => {
  ...
  // Steps: 
  // Expected: Journal Agent — appears in both sections under the identical name, with a full feature description in the top grid. Direct, unambiguous contradiction. E-commerce Shop — doesn't appear by this exact name in the top grid, but "Shopping Agent" does, describing the same underlying marketplace/product feature. This is the same feature listed under two different names in two different status sections — a contradiction compounded with the separately-confirmed naming-inconsistency issue (NAME-CONSIST-01). Financial Wellness — does not appear anywhere in the live top grid. No contradiction for this specific item.
  test("Service_01: Coming Soon list contradicts the live Services grid", async ({ page }) => {
    await gotoPublic(page, '/services');
    await expect(page.getByText('Journal Agent', { exact: true }).first()).toBeVisible();
    await expect(page.getByText('Shopping Agent', { exact: true }).first()).toBeVisible();
    await expect(page.getByText('Financial Wellness', { exact: true })).toHaveCount(0);
  });
